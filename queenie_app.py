...
with _c1:
    st.markdown(f"""<div class="top-bar-col" style="color:white;">
        <div style="font-size:13px; opacity:0.85; margin-bottom:4px;">距 {EXAM_YEAR} 學測</div>
        <div style="font-size:32px; font-weight:700; line-height:1.1; color:white;">{phase_info['days_left']} <span style="font-size:14px; font-weight:400; opacity:0.85;">天</span></div>
    </div>""", unsafe_allow_html=True)
with _c2:
    st.markdown(f"""<div class="top-bar-col" style="color:white; text-align:center;">
        <div style="font-size:13px; opacity:0.85; margin-bottom:6px;">{phase_info['phase']}</div>
        <div style="font-size:13px; font-weight:600; background:rgba(255,255,255,0.2); padding:5px 12px; border-radius:99px; display:inline-block; color:white;">{phase_info['focus']}</div>
    </div>""", unsafe_allow_html=True)
with _c3:
    st.markdown(f"""<div class="top-bar-col" style="color:white; text-align:right;">
        <div style="font-size:13px; opacity:0.85; margin-bottom:4px;">每日目標</div>
        <div style="font-size:32px; font-weight:700; line-height:1.1; color:white;">{phase_info['daily_target']} <span style="font-size:14px; font-weight:400; opacity:0.85;">題</span></div>
    </div>""", unsafe_allow_html=True)

# 進度條（獨立一行）
_prog = min(phase_info['progress_pct'], 100)
st.markdown(f"""
<div style="background:#1e3a5f; border-radius:0 0 8px 8px; padding:0 16px 12px; margin-top:-12px; margin-bottom:14px;">
    <div style="background:rgba(255,255,255,0.2); border-radius:99px; height:4px;">
