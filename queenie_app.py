...
<style>
    ...
    [data-testid="stMetricLabel"]  { font-size: 12px !important; color: var(--text-muted) !important; }
...
st.markdown(f"""
<div style="background:#1e3a5f; color:white; padding:10px 16px; border-radius:8px; margin-bottom:12px;">
    <div style="display:flex; justify-content:space-between; align-items:center; flex-wrap:wrap; gap:6px;">
        <div style="display:flex; align-items:baseline; gap:6px;">
            <span style="font-size:22px; font-weight:600; line-height:1;">{phase_info['days_left']}</span>
            <span style="font-size:12px; opacity:0.75;">天 · {EXAM_YEAR} 學測</span>
        </div>
        <div style="display:flex; align-items:center; gap:8px;">
            <span style="font-size:11px; background:rgba(255,255,255,0.12); padding:3px 8px; border-radius:99px;">{phase_info['phase']}</span>
            <span style="font-size:11px; opacity:0.75;">{phase_info['focus']}</span>
        </div>
        <div style="text-align:right;">
            <span style="font-size:18px; font-weight:600;">{phase_info['daily_target']}</span>
            <span style="font-size:11px; opacity:0.75;"> 題/日</span>
        </div>
    </div>
    <div style="margin-top:6px; background:rgba(255,255,255,0.15); border-radius:99px; height:3px;">
        <div style="width:{phase_info['progress_pct']}%; height:100%; background:rgba(255,255,255,0.7); border-radius:99px;"></div>
    </div>
</div>
""", unsafe_allow_html=True)
        ...
        with col_btn1:
            if st.button("🚀 開始考試", type="primary", use_container_width=True):
                if not chapter or chapter == "✏️ 自訂":
                    ...
                    prompt = f"""你是台灣高中升大學家教，熟悉 108 課綱。
...
"""
                    with st.spinner(f"{engine} 正在出題中，請稍候..."):
                        ...
                        try:
                            ...
                            st.error(f"❌ AI 出題格式有誤，請再試一次（或切換 AI 引擎）")
