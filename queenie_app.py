    ...
    # ===== 階段 2：作答中 =====
        ...
        for q in questions:
            ...
            if q_type == "multiple_choice":
                ...
                # 單選：用 st.radio 但隱藏原生圓圈，自行用 HTML 渲染方框
                current = st.session_state.user_answers.get(qid)
                for k in opt_keys:
                    is_selected = (current == k)
                    box = "☑" if is_selected else "☐"
                    col_box, col_label = st.columns([0.04, 0.96])
                    with col_box:
                        if st.button(box, key=f"q_{qid}_{k}_btn",
                                     help=None,
                                     use_container_width=False):
                            if is_selected:
                                st.session_state.user_answers[qid] = None
                            else:
                                st.session_state.user_answers[qid] = k
                            st.rerun()
                    with col_label:
                        weight = "bold" if is_selected else "normal"
                        color = "#1e3a5f" if is_selected else "#111827"
                        st.markdown(f'<span style="font-size:15px;font-weight:{weight};color:{color};">{k}. {opts[k]}</span>', unsafe_allow_html=True)
