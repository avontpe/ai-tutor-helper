"""
AI Tutor Helper v3.8.8（修正電腦版橫條截斷＋手機版側邊欄收合）
================================
...
st.set_page_config(
    ...
    page_icon="🎓",
    layout="wide",
    initial_sidebar_state="auto",  # 桌機展開、手機依螢幕自動決定
)
...
<style>
    ...
    /* ── 確保頂部橫條不被截斷（v3.8.8 修正）── */
    div[data-testid="stMarkdownContainer"]:has(div[style*="background:#1e3a5f"]) {
    ...
    h3 { font-size: 19px !important; font-weight: 600 !important;
         color: var(--text) !important; margin-bottom: 4px !important; }
    h4, h5 { font-size: 17px !important; font-weight: 500 !important;
...
def init_db():
    with get_db() as conn:
        conn.executescript("""
        CREATE TABLE IF NOT EXISTS quiz_draft (
...
def check_app_password():
    """主 App 登入密碼驗證（query_params 持久化，重整不登出）"""
    if not APP_PASSWORD:
        return True

    # 從 query_params 恢復登入狀態與頁面（重整後仍有效）
    params = st.query_params
    if params.get("auth") == "ok":
        st.session_state.app_authenticated = True
    # 恢復上次所在頁面
    if "pg" in params and "current_page" not in st.session_state:
        st.session_state.current_page = params["pg"]

    if st.session_state.get("app_authenticated"):
        return True

    # 登入畫面（全新設計）
    _login_days = (EXAM_DATE - date.today()).days
    st.markdown(f"""
    <style>
    .login-wrap {{
        max-width: 520px;
    ...
    <div class="login-wrap">
        <div class="login-hero">
            <div class="login-badge">📅 距 116 學測 {_login_days} 天</div>
            <div style="font-size:60px; margin:8px 0; position:relative; z-index:1;">🎓</div>
            <div style="font-size:30px; font-weight:700; letter-spacing:-0.5px; position:relative; z-index:1;">學測導航</div>
            <div style="font-size:14px; opacity:0.9; margin-top:8px; position:relative; z-index:1;">
                你的 AI 備考夥伴 · 輸入密碼開始學習
            </div>
            <div style="display:flex; justify-content:center; gap:16px; margin-top:16px; position:relative; z-index:1;">
                <div style="text-align:center; background:rgba(255,255,255,0.2); padding:8px 16px; border-radius:8px;">
                    <div style="font-size:18px; font-weight:700;">5 科</div>
                    <div style="font-size:11px; opacity:0.85;">學測科目</div>
                </div>
                <div style="text-align:center; background:rgba(255,255,255,0.2); padding:8px 16px; border-radius:8px;">
                    <div style="font-size:18px; font-weight:700;">AI</div>
                    <div style="font-size:11px; opacity:0.85;">智慧出題</div>
                </div>
                <div style="text-align:center; background:rgba(255,255,255,0.2); padding:8px 16px; border-radius:8px;">
                    <div style="font-size:18px; font-weight:700;">∞</div>
                    <div style="font-size:11px; opacity:0.85;">刷題無限</div>
                </div>
            </div>
        </div>
        <div class="login-body">
            <div style="font-size:14px; font-weight:500; color:#374151; margin-bottom:8px;">🔑 請輸入密碼</div>
        </div>
    </div>
    """, unsafe_allow_html=True)
    ...
    with col2:
        ...
        if st.button("🔓  進入 App", type="primary", use_container_width=True):
            if password == APP_PASSWORD:
                ...
                st.error("❌ 密碼錯誤，請再試一次")
        st.markdown('<div class="login-hint">如有問題請聯繫家長</div>', unsafe_allow_html=True)
...
with st.sidebar:
    ...
    # 優先從 query_params 恢復頁面，其次 session_state
    default_page_idx = 0
    _restore_page = st.query_params.get("pg") or st.session_state.get("current_page", "🏠 首頁")
    if _restore_page in PAGE_OPTIONS:
        default_page_idx = PAGE_OPTIONS.index(_restore_page)
...
# ============================================================
# 9. 頂部資訊欄（用 Streamlit 原生排版，避免 HTML 截斷）
# ============================================================

# ============================================================
# 9. 頂部資訊欄（純 Streamlit columns + CSS 背景）
# ============================================================

# 頂部資訊欄：全部用一個 st.markdown 搞定，絕對無縫
_prog = min(phase_info['progress_pct'], 100)
st.markdown(f"""
<div style="background:#1e3a5f; border-radius:8px; padding:16px 20px 14px; margin-bottom:8px; margin-top:4px;">
    <div style="display:flex; justify-content:space-between; align-items:flex-start; margin-bottom:12px;">
        <div style="flex:3;">
            <div style="font-size:13px; color:rgba(255,255,255,0.8); margin-bottom:3px;">距 {EXAM_YEAR} 學測</div>
            <div style="font-size:34px; font-weight:700; line-height:1; color:white;">{phase_info['days_left']}<span style="font-size:15px; font-weight:400; margin-left:3px; opacity:0.85;">天</span></div>
        </div>
        <div style="flex:4; text-align:center;">
            <div style="font-size:13px; color:rgba(255,255,255,0.8); margin-bottom:5px;">{phase_info['phase']}</div>
            <div style="font-size:13px; font-weight:600; background:rgba(255,255,255,0.2); padding:4px 12px; border-radius:99px; display:inline-block; color:white;">{phase_info['focus']}</div>
        </div>
        <div style="flex:3; text-align:right;">
            <div style="font-size:13px; color:rgba(255,255,255,0.8); margin-bottom:3px;">每日目標</div>
            <div style="font-size:34px; font-weight:700; line-height:1; color:white;">{phase_info['daily_target']}<span style="font-size:15px; font-weight:400; margin-left:3px; opacity:0.85;">題</span></div>
        </div>
    </div>
    <div style="background:rgba(255,255,255,0.15); border-radius:99px; height:5px;">
        <div style="width:{_prog}%; height:100%; background:rgba(255,255,255,0.85); border-radius:99px;"></div>
    </div>
</div>
""", unsafe_allow_html=True)
# ============================================================
...
# 右上角操作列（所有頁面都顯示）
_top_col1, _top_col2, _top_col3 = st.columns([5, 1, 1])
with _top_col2:
    if page != "🏠 首頁":
        if st.button("🏠 首頁", key="back_home_top", use_container_width=True):
            ...
            st.rerun()
with _top_col3:
    if st.button("🚪 登出", key="logout_top", use_container_width=True):
        st.query_params.clear()
        # 登出時清除作答草稿，下次登入是全新狀態
        try:
            with get_db() as _c:
                _c.execute("DELETE FROM quiz_draft WHERE id=1")
        except Exception:
            pass
...
if page == "🏠 首頁":
    ...
    avg_score = round(sum(s["score"] for s in all_scores[:20]) / len(all_scores[:20])) if all_scores else 0

    # ── 歡迎標題 ──
    st.markdown(f"""
    <div style="margin-bottom:16px;">
        <div style="font-size:22px; font-weight:600; color:#111827;">Hi {STUDENT_NAME[:1]}，今天該做什麼？</div>
        <div style="font-size:13px; color:#6b7280; margin-top:2px;">{date.today().strftime('%Y 年 %m 月 %d 日')} · {phase_info['phase']}</div>
    </div>
    """, unsafe_allow_html=True)
    ...
    done_color = "#4a7c59" if total_done >= phase_info["daily_target"] else "#1e3a5f"
    with col1:
        st.markdown(f"""
        <div style="background:white; border:1px solid #d6d6d8; border-radius:6px; padding:14px; border-left:3px solid {done_color};">
            <div style="font-size:12px; color:#6b7280;">📝 今日題數</div>
            <div style="font-size:28px; font-weight:600; color:{done_color}; line-height:1.2;">{total_done}</div>
            <div style="font-size:12px; color:#6b7280;">目標 {phase_info['daily_target']} 題</div>
        </div>
        """, unsafe_allow_html=True)

    time_color = "#4a7c59" if total_mins >= 30 else "#1e3a5f"
    with col2:
        st.markdown(f"""
        <div style="background:white; border:1px solid #d6d6d8; border-radius:6px; padding:14px; border-left:3px solid {time_color};">
            <div style="font-size:12px; color:#6b7280;">⏰ 今日時間</div>
            <div style="font-size:28px; font-weight:600; color:{time_color}; line-height:1.2;">{total_mins}</div>
            <div style="font-size:12px; color:#6b7280;">分鐘</div>
        </div>
        """, unsafe_allow_html=True)

    comp_color = "#4a7c59" if completion >= 100 else "#c08827" if completion >= 50 else "#1e3a5f"
    with col3:
        st.markdown(f"""
        <div style="background:white; border:1px solid #d6d6d8; border-radius:6px; padding:14px; border-left:3px solid {comp_color};">
            <div style="font-size:12px; color:#6b7280;">✅ 今日完成度</div>
            <div style="font-size:28px; font-weight:600; color:{comp_color}; line-height:1.2;">{completion}%</div>
            <div style="font-size:12px; color:#6b7280;">{'🎉 達標！' if completion >= 100 else '💪 加油！'}</div>
        </div>
        """, unsafe_allow_html=True)

    streak_color = "#d97757" if streak > 0 else "#6b7280"
    with col4:
        st.markdown(f"""
        <div style="background:white; border:1px solid #d6d6d8; border-radius:6px; padding:14px; border-left:3px solid {streak_color};">
            <div style="font-size:12px; color:#6b7280;">🔥 連續達標</div>
            <div style="font-size:28px; font-weight:600; color:{streak_color}; line-height:1.2;">{streak}</div>
            <div style="font-size:12px; color:#6b7280;">天</div>
        </div>
        """, unsafe_allow_html=True)
    ...
    # ── 今日進度條 ──
    pct = min(completion, 100)
    bar_color = "#4a7c59" if pct >= 100 else "#1e3a5f"
    st.markdown(f"""
    <div style="background:white; border:1px solid #d6d6d8; border-radius:6px; padding:14px; margin-bottom:14px;">
        <div style="display:flex; justify-content:space-between; margin-bottom:8px;">
            <span style="font-size:13px; font-weight:500; color:#111827;">今日進度</span>
            <span style="font-size:13px; color:{bar_color}; font-weight:600;">{total_done} / {phase_info['daily_target']} 題</span>
        </div>
        <div style="background:#e5e7eb; border-radius:99px; height:8px;">
            <div style="width:{pct}%; background:{bar_color}; height:100%; border-radius:99px; transition:width 0.4s;"></div>
        </div>
    </div>
    """, unsafe_allow_html=True)
    ...
    st.markdown("""
    <div style="font-size:14px; font-weight:600; color:#111827; margin-bottom:8px;">📋 今日建議</div>
    """, unsafe_allow_html=True)

    if weakness_subjects:
        items_html = "".join([
            f'<div style="display:flex; justify-content:space-between; padding:8px 0; border-bottom:1px solid #f3f4f6;">'
            f'<span style="font-size:13px; color:#111827;">🎯 優先複習：<b>{s}</b></span>'
            f'<span style="font-size:12px; background:#fee2e2; color:#991b1b; padding:2px 8px; border-radius:99px;">{cnt} 題待複習</span>'
            f'</div>'
            for s, cnt in weakness_subjects[:3]
        ])
        st.markdown(f"""
        <div style="background:white; border:1px solid #d6d6d8; border-radius:6px; padding:12px 16px; margin-bottom:14px;">
            {items_html}
        </div>
        """, unsafe_allow_html=True)
        col_q1, col_q2 = st.columns(2)
        with col_q1:
            if st.button("📝 去刷題練習", type="primary", use_container_width=True):
                st.session_state.current_page = "📝 刷題練習"
                if "page_radio" in st.session_state:
                    del st.session_state.page_radio
                st.rerun()
        with col_q2:
            if st.button("📓 去複習錯題", use_container_width=True):
        ...
        st.markdown(f"""
        <div style="background:#f0fdf4; border:1px solid #bbf7d0; border-radius:6px; padding:12px 16px; margin-bottom:14px;">
            <span style="font-size:13px; color:#166534;">👍 目前沒有待複習錯題！去「📝 刷題練習」開始今日 {phase_info['daily_target']} 題吧！</span>
        </div>
        """, unsafe_allow_html=True)
        if st.button("📝 開始今日練習", type="primary", use_container_width=True):
            st.session_state.current_page = "📝 刷題練習"
            if "page_radio" in st.session_state:
                del st.session_state.page_radio
            st.rerun()
    ...
    if get_reward_enabled():
        ...
        ach_color = "#4a7c59" if ach["is_great"] else "#c08827" if ach["is_good"] else "#1e3a5f"
        items_html = "".join([
            f'<div style="display:flex; justify-content:space-between; padding:6px 0;">'
            f'<span style="font-size:13px; color:#111827;">{"✅" if c["achieved"] else "⏳"} {c["name"]}</span>'
            f'<span style="font-size:12px; color:#6b7280;">{c["detail"]}</span>'
            f'</div>'
            for c in ach["conditions"]
        ])
        st.markdown(f"""
        <div style="background:white; border:1px solid #d6d6d8; border-radius:6px; padding:14px; margin-bottom:14px; border-left:3px solid {ach_color};">
            <div style="font-size:13px; font-weight:600; color:{ach_color}; margin-bottom:8px;">🏆 本月達標進度：{ach['achieved_count']} / 4</div>
            {items_html}
        </div>
        """, unsafe_allow_html=True)
    ...
    # ── 登錄今日進度 ──
    st.markdown("""
    <div style="font-size:14px; font-weight:600; color:#111827; margin:14px 0 8px;">✏️ 登錄今日進度</div>
    """, unsafe_allow_html=True)
...
# ---------- 📝 刷題練習 (v3.7 真考試模式) ----------
    ...
    # 狀態管理：每次進入此頁都嘗試從 DB 讀取草稿
    # （不限 not in session_state，確保重整後也能恢復）
    if st.session_state.get("quiz_state") in (None, "setup", "") or        st.session_state.get("quiz_data") is None:
        with get_db() as _conn:
            _row = _conn.execute(
                "SELECT data FROM quiz_draft WHERE id=1"
            ).fetchone()
        if _row:
            try:
                _draft = json.loads(_row["data"])
                _qs = _draft.get("quiz_state", "setup")
                _qd = _draft.get("quiz_data")
                # key 從 string 轉回 int（JSON 序列化造成）
                _ua_raw = _draft.get("user_answers", {})
                _ua = {}
                for k, v in _ua_raw.items():
                    try:
                        _ua[int(k)] = v
                    except Exception:
                        _ua[k] = v
                if _qs == "answering" and _qd:
                    st.session_state.quiz_state      = _qs
                    st.session_state.quiz_data       = _qd
                    st.session_state.user_answers    = _ua
                    st.session_state.quiz_subject    = _draft.get("quiz_subject", "")
                    st.session_state.quiz_topic      = _draft.get("quiz_topic", "")
                    st.session_state.quiz_time_limit = _draft.get("quiz_time_limit", 0)
                    if "quiz_start_time" not in st.session_state:
                        st.session_state.quiz_start_time = datetime.now()
                else:
                    st.session_state.setdefault("quiz_state", "setup")
...
請為高中生 {NICKNAME} 出 {num_q} 題「{subject}」練習題。{boost_hint}
其中約 80% 為單選題、約 20% 為複選題（仿真實學測比例，{num_q} 題中約出 1～2 題複選）。
...
{{
  ...
  "questions": [
    {{
      ...
      "answer": "B",
      "explanation": "詳解：解題思路、易錯點"
    }},
    {{
      "id": 2,
      "type": "multiple_choice",
      "question": "【多選題】完整題目敘述...（以下選項有 2～3 個正確答案，每題答對得2分，答錯0分，漏選得1分）",
      "options": {{
        "A": "選項 A 內容",
        "B": "選項 B 內容",
        "C": "選項 C 內容",
      ...
      "answer": ["A", "C"],
      "explanation": "詳解：說明每個正確選項的理由，及錯誤選項為何錯"
    }}
  ]
}}

【出題原則】
1. single_choice：4 選 1，answer 填單一字母（如 "B"）
2. multiple_choice：5 選 2～3，answer 填字母陣列（如 ["A","C"]），題目須標示「複選題」
3. 仿學測 108 課綱，情境化、有思考深度
4. explanation 詳細說明選項邏輯
5. 題目不重複，難度遞進
6. **絕對不要在 JSON 外加任何文字**
"""
    ...
    # ===== 階段 2：作答中 =====
        ...
        for q in questions:
            ...
            if q_type == "multiple_choice":
                st.markdown(f"##### 第 {qid} 題　　🔲 **多選題**")
                st.markdown('<div style="background:#fff3cd; border-left:4px solid #d97706; padding:6px 12px; border-radius:4px; margin:4px 0 10px; font-size:14px; color:#92400e; font-weight:600;">✏️ 多選題：請勾選所有正確答案　|　全對 +2分・漏選 +1分・錯選 +0分</div>', unsafe_allow_html=True)
            else:
            ...
            if q_type == "multiple_choice":
                # 複選：用 checkbox
                current_multi = st.session_state.user_answers.get(qid, [])
                ...
                try:
                    _draft_data = json.dumps({
                        ...
                        "quiz_time_limit": st.session_state.get("quiz_time_limit", 0),
                    }, ensure_ascii=False)
                    with get_db() as _c:
                        _c.execute("INSERT OR REPLACE INTO quiz_draft (id, data, updated_at) VALUES (1, ?, ?)",
                                   (_draft_data, datetime.now().isoformat()))
                except Exception:
                    pass
            else:
                # 單選：回歸乾淨的 st.radio，CSS 統一圓圈樣式
                current = st.session_state.user_answers.get(qid)
                choice = st.radio(
                    "選擇答案",
                    options=opt_keys,
                    format_func=lambda x, o=opts: f"{x}. {o.get(x, '')}",
                    key=f"q_{qid}",
                    index=opt_keys.index(current) if current in opt_keys else None,
                    label_visibility="collapsed",
                )
                if choice:
    ...
    # ===== 階段 3：已交卷，評分 + 詳解 =====
        ...
        for q in questions:
            ...
            user_a = user_ans.get(qid)
            if q_type == "multiple_choice":
                # 複選學測計分：全對+2、漏選+1（無錯選）、有錯選+0
                correct_set = set(correct_ans) if isinstance(correct_ans, list) else {correct_ans}
                user_set = set(user_a) if isinstance(user_a, list) else set()
                wrong_picks = user_set - correct_set      # 選錯的
                missed = correct_set - user_set           # 漏選的
                if not wrong_picks and not missed:
                    correct_count += 2    # 全對：+2
                    is_correct = True
                elif not wrong_picks and missed:
                    correct_count += 1    # 漏選無錯：+1
                    is_correct = False
                else:
                    is_correct = False    # 有錯選：+0
            else:
                is_correct = (user_a == correct_ans)
                if is_correct:
                    correct_count += 1
            if not is_correct:
...
# ---------- 🔐 家長後台 ----------
    ...
    # 💾 資料備份（移至家長後台）
    st.subheader("💾 資料備份")
    st.warning("⚠️ Streamlit Cloud 重啟可能會清空資料，建議每週下載備份一次。")
    ...
    with col_b1:
        st.metric("錯題本", f"{wrong_cnt} 題")
    with col_b2:
        st.metric("分數紀錄", f"{score_cnt} 筆")
    with col_b3:
        st.metric("每日紀錄", f"{log_cnt} 筆")

    backup_data = export_db_json()
    st.download_button(
        "⬇️ 下載完整備份（JSON）",
        backup_data,
        file_name=f"queenie_backup_{date.today()}.json",
        mime="application/json",
        type="primary",
        use_container_width=True,
    )
    st.caption("💡 備份包含：所有錯題、分數記錄、每日進度。請存放在安全的地方。")

    st.divider()

    # 🧹 清空測試資料
    st.subheader("🧹 清空測試資料")
    st.markdown('<div style="background:#fef2f2;border-left:4px solid #dc2626;border-radius:6px;padding:10px 14px;color:#991b1b;font-size:13px;">⚠️ 此操作不可復原！正式使用前清空一次，之後請勿再按。</div>', unsafe_allow_html=True)
    st.markdown("")

    col_r1, col_r2, col_r3, col_r4 = st.columns(4)
    with col_r1:
        if st.button("🗑️ 清空錯題本", use_container_width=True):
            with get_db() as conn:
                conn.execute("DELETE FROM wrong_book")
            st.success("✅ 錯題本已清空")
            st.rerun()
    with col_r2:
        if st.button("🗑️ 清空分數記錄", use_container_width=True):
            with get_db() as conn:
                conn.execute("DELETE FROM score_log")
            st.success("✅ 分數記錄已清空")
            st.rerun()
    with col_r3:
        if st.button("🗑️ 清空每日紀錄", use_container_width=True):
            with get_db() as conn:
    ...
    with col_r4:
        if st.button("🗑️ 全部清空", type="primary", use_container_width=True):
            with get_db() as conn:
                ...
                conn.execute("DELETE FROM score_log")
                conn.execute("DELETE FROM daily_log")
                conn.execute("DELETE FROM quiz_draft WHERE id=1")
            st.success("✅ 所有測試資料已清空，Queenie 可以全新開始！")
            st.rerun()

    st.divider()

    with st.expander("📱 把 App 加到 Mac 或 iPad 主畫面"):
        st.markdown("""
        **Mac（Chrome 或 Safari）：**
        1. 打開 https://ai-study-app.streamlit.app
        2. 點瀏覽器右上角 **⋯** 或 **分享**
        3. 選「**加入書籤**」或「**建立捷徑**」
...
# 頁尾
st.divider()
st.caption(f"🎓 v3.8.1 | {engine} | {publisher}版 {grade}{subject} | 距學測 {phase_info['days_left']} 天")
